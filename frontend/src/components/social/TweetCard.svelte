...
<article class="p-4 border-b {isDarkMode ? 'border-gray-800' : 'border-gray-200'} hover:{isDarkMode ? 'bg-gray-800/50' : 'bg-gray-100/50'} transition-colors cursor-pointer">
  <div class="flex">
    ...
    <div class="ml-3 flex-1 overflow-hidden">
      <div class="flex items-center">
        ...
        <span class="{isDarkMode ? 'text-gray-400' : 'text-gray-500'} ml-2 text-sm">@{tweet.username}</span>
        <span class="{isDarkMode ? 'text-gray-400' : 'text-gray-500'} mx-1 text-sm">·</span>
        <span class="{isDarkMode ? 'text-gray-400' : 'text-gray-500'} text-sm hover:underline">{tweet.timestamp}</span>
      </div>
      
      <p class="mt-1 mb-2 whitespace-pre-wrap break-words">{tweet.content}</p>
      
      <div class="flex justify-between mt-3 {isDarkMode ? 'text-gray-400' : 'text-gray-500'} max-w-md">
        <button class="group flex items-center hover:text-blue-500 transition-colors">
          ...
          <span class="ml-1 text-xs">{tweet.replies}</span>
        </button>
        <button class="group flex items-center hover:text-green-500 transition-colors" on:click={repostTweet}>
          ...
          <span class="ml-1 text-xs {tweet.reposted ? 'text-green-500' : ''}">{tweet.reposts}</span>
        </button>
        <button class="group flex items-center hover:text-pink-500 transition-colors" on:click={likeTweet}>
          ...
          <span class="ml-1 text-xs {tweet.liked ? 'text-pink-500' : ''}">{tweet.likes}</span>
        </button>
        <button class="group flex items-center hover:text-blue-500 transition-colors">
          <div class="p-2 rounded-full group-hover:bg-blue-500/10 transition-colors">
            <EyeIcon size="16" />
          </div>
          <span class="ml-1 text-xs">{tweet.views}</span>
        </button>

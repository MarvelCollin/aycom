  ...
  <a href={`/profile/${profile.username}`} class="flex items-center flex-1 min-w-0">
    ...
    <div class="flex-1 min-w-0">
      <div class="flex items-center">
        <p class="font-bold {isDarkMode ? 'text-white' : 'text-black'} truncate">{profile.displayName}</p>
      ...
      {/if}
      {#if showFollowerCount && profile.followerCount > 0}
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
          <span class="font-semibold">{profile.followerCount}</span> {profile.followerCount === 1 ? 'follower' : 'followers'}
        </p>
      {/if}
    </div>
  </a>
  <button 
    class="ml-4 px-4 py-1.5 rounded-full {profile.isFollowing ? 'bg-transparent border border-gray-300 dark:border-gray-600' : 'bg-black dark:bg-white text-white dark:text-black'} font-bold text-sm"

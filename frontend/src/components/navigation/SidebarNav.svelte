...
<div class="h-screen w-[68px] md:w-[275px] py-1 overflow-y-auto">
  <div class="flex flex-col items-center md:items-start h-full px-2">
    <!-- Logo -->
    <div class="p-3 rounded-full {isDarkMode ? 'hover:bg-gray-800/70' : 'hover:bg-gray-200/70'} transition-colors w-fit">
    ...
    <nav class="w-full mt-2">
      {#each navigationItems as item}
        ...
        <a 
          href={item.path} 
          class="flex items-center my-1 p-3 {isDarkMode ? 'hover:bg-gray-800/70' : 'hover:bg-gray-200/70'} {currentPath === item.path ? 'font-bold' : ''} rounded-full group transition-colors"
        >
          <div class="flex items-center justify-center w-6 h-6">
            <svelte:component this={IconComponent} size="24" />
          </div>
          <span class="hidden md:inline text-xl ml-4 {currentPath === item.path ? 'font-bold' : ''}">{item.label}</span>
        </a>
      {/each}
    </nav>
    ...
    <button 
      class="flex items-center p-3 mt-4 {isDarkMode ? 'hover:bg-gray-800/70' : 'hover:bg-gray-200/70'} rounded-full mb-1 w-full transition-colors"
    ...
    >
      ...
      <span class="hidden md:inline text-xl ml-4">Theme</span>
    </button>
    
    <!-- User Profile -->
    <div class="mt-auto mb-4 w-full">
      <div 
        class="flex items-center p-3 {isDarkMode ? 'hover:bg-gray-800/70' : 'hover:bg-gray-200/70'} rounded-full cursor-pointer transition-colors"
        on:click={toggleUserMenu}
      >
        <div class="w-10 h-10 {isDarkMode ? 'bg-gray-700' : 'bg-gray-300'} rounded-full flex items-center justify-center transition-colors overflow-hidden">
          <span>{avatar}</span>
        </div>

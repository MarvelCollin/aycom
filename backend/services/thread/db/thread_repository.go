...
type ThreadRepository interface {
	...
	FindThreadByID(id string) (*Thread, error)
	FindThreadsByUserID(userID string, page, limit int) ([]*Thread, int64, error)
	FindThreadsByCommunityID(communityID string, page, limit int) ([]*Thread, int64, error)
...
type ReplyRepository interface {
	...
	FindReplyByID(id string) (*Reply, error)
	FindRepliesByThreadID(threadID string, parentReplyID *string, page, limit int) ([]*Reply, int64, error)
...
type BookmarkRepository interface {
	...
	GetUserBookmarks(userID string, page, limit int) ([]*Thread, int64, error)
}

// MediaRepository handles database operations for media
type MediaRepository interface {
	CreateMedia(media *Media) error
	FindMediaByID(id string) (*Media, error)
	FindMediaByThreadID(threadID string) ([]*Media, error)
	FindMediaByReplyID(replyID string) ([]*Media, error)
	DeleteMedia(id string) error
}

// HashtagRepository handles database operations for hashtags
type HashtagRepository interface {
	CreateHashtag(hashtag *Hashtag) error
	FindOrCreateHashtagByText(text string) (*Hashtag, error)
	FindHashtagByID(id string) (*Hashtag, error)
	FindHashtagByText(text string) (*Hashtag, error)
	AddHashtagToThread(threadID, hashtagID string) error
	RemoveHashtagFromThread(threadID, hashtagID string) error
	FindHashtagsByThreadID(threadID string) ([]*Hashtag, error)
	FindTrendingHashtags(limit int) ([]*Hashtag, int64, error)
}

// MentionRepository handles database operations for user mentions
type MentionRepository interface {
	CreateMention(mention *UserMention) error
	FindMentionsByThreadID(threadID string) ([]*UserMention, error)
	FindMentionsByReplyID(replyID string) ([]*UserMention, error)
...
type CategoryRepository interface {
	...
	FindOrCreateCategoryByName(name string, categoryType string) (*Category, error)
	FindCategoryByID(id string) (*Category, error)
	FindCategoryByName(name string, categoryType string) (*Category, error)
	AddCategoryToThread(threadID, categoryID string) error
	RemoveCategoryFromThread(threadID, categoryID string) error
	FindCategoriesByThreadID(threadID string) ([]*Category, error)
}

// PollRepository handles database operations for polls
type PollRepository interface {
	CreatePoll(poll *Poll) error
	CreatePollOptions(options []*PollOption) error
	FindPollByID(id string) (*Poll, error)
	FindPollByThreadID(threadID string) (*Poll, error)
	FindPollOptionByID(id string) (*PollOption, error)
	FindPollOptionsByPollID(pollID string) ([]*PollOption, error)
	CreateVote(vote *PollVote) error
	DeleteVote(userID, pollID string) error
	FindVoteByUserAndPoll(userID, pollID string) (*PollVote, error)

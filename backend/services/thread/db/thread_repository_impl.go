...
func (r *threadRepositoryImpl) CreateThread(thread *Thread) error {
	return r.db.Create(thread).Error
}

// FindThreadByID implements ThreadRepository
func (r *threadRepositoryImpl) FindThreadByID(id string) (*Thread, error) {
	var thread Thread
...
func (r *threadRepositoryImpl) FindThreadsByUserID(userID string, page, limit int) ([]*Thread, int64, error) {
	var threads []*Thread
...
func (r *threadRepositoryImpl) FindThreadsByCommunityID(communityID string, page, limit int) ([]*Thread, int64, error) {
	var threads []*Thread
	var totalCount int64
...
func (r *threadRepositoryImpl) UpdateThread(thread *Thread) error {
...
func (r *threadRepositoryImpl) DeleteThread(id string) error {
	return r.db.Delete(&Thread{}, "thread_id = ?", id).Error
}
...
func (r *mediaRepositoryImpl) CreateMedia(media *Media) error {
	return r.db.Create(media).Error
}

// FindMediaByID implements MediaRepository
func (r *mediaRepositoryImpl) FindMediaByID(id string) (*Media, error) {
	var media Media
...
func (r *mediaRepositoryImpl) FindMediaByThreadID(threadID string) ([]*Media, error) {
	var mediaList []*Media
...
func (r *mediaRepositoryImpl) FindMediaByReplyID(replyID string) ([]*Media, error) {
	var mediaList []*Media
...
func (r *mediaRepositoryImpl) DeleteMedia(id string) error {
	return r.db.Delete(&Media{}, "media_id = ?", id).Error
}
...
func (r *hashtagRepositoryImpl) CreateHashtag(hashtag *Hashtag) error {
	return r.db.Create(hashtag).Error
}

// FindOrCreateHashtagByText implements HashtagRepository
func (r *hashtagRepositoryImpl) FindOrCreateHashtagByText(text string) (*Hashtag, error) {
	var hashtag Hashtag
...
func (r *hashtagRepositoryImpl) FindHashtagByID(id string) (*Hashtag, error) {
	var hashtag Hashtag
...
func (r *hashtagRepositoryImpl) FindHashtagByText(text string) (*Hashtag, error) {
	var hashtag Hashtag
	err := r.db.Where("text = ?", text).First(&hashtag).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
...
func (r *hashtagRepositoryImpl) RemoveHashtagFromThread(threadID, hashtagID string) error {
	return r.db.Where("thread_id = ? AND hashtag_id = ?", threadID, hashtagID).Delete(&ThreadHashtag{}).Error
}

// FindHashtagsByThreadID implements HashtagRepository
func (r *hashtagRepositoryImpl) FindHashtagsByThreadID(threadID string) ([]*Hashtag, error) {
...
func (r *hashtagRepositoryImpl) FindTrendingHashtags(limit int) ([]*Hashtag, int64, error) {
	var hashtags []*Hashtag
	var totalCount int64

	// Get total count
	if err := r.db.Model(&Hashtag{}).Count(&totalCount).Error; err != nil {
		return nil, 0, err
	}
...
func (r *mentionRepositoryImpl) CreateMention(mention *UserMention) error {
	return r.db.Create(mention).Error
}

// FindMentionsByThreadID implements MentionRepository
func (r *mentionRepositoryImpl) FindMentionsByThreadID(threadID string) ([]*UserMention, error) {
...
func (r *mentionRepositoryImpl) FindMentionsByReplyID(replyID string) ([]*UserMention, error) {
	var mentions []*UserMention
...
func (r *mentionRepositoryImpl) DeleteMention(id string) error {
	return r.db.Delete(&UserMention{}, "mention_id = ?", id).Error
}
...
func (r *replyRepositoryImpl) CreateReply(reply *Reply) error {
	return r.db.Create(reply).Error
}

// FindReplyByID implements ReplyRepository
func (r *replyRepositoryImpl) FindReplyByID(id string) (*Reply, error) {
	var reply Reply
...
func (r *replyRepositoryImpl) FindRepliesByThreadID(threadID string, parentReplyID *string, page, limit int) ([]*Reply, int64, error) {
	var replies []*Reply
	var totalCount int64

	offset := (page - 1) * limit
	query := r.db.Model(&Reply{}).Where("thread_id = ?", threadID)
...
func (r *replyRepositoryImpl) GetReplyStats(replyID string) (replyCount, likeCount int64, err error) {
	...
	if err := r.db.Model(&Reply{}).Where("parent_reply_id = ?", replyID).Count(&replyCount).Error; err != nil {
		return 0, 0, err
	}

	// Count likes on this reply
	if err := r.db.Model(&Like{}).Where("reply_id = ?", replyID).Count(&likeCount).Error; err != nil {
		return 0, 0, err
	}
...
func (r *likeRepositoryImpl) DeleteThreadLike(userID, threadID string) error {
	return r.db.Where("user_id = ? AND thread_id = ?", userID, threadID).Delete(&Like{}).Error
}
...
func (r *likeRepositoryImpl) DeleteReplyLike(userID, replyID string) error {
	return r.db.Where("user_id = ? AND reply_id = ?", userID, replyID).Delete(&Like{}).Error
}
...
func (r *likeRepositoryImpl) HasUserLikedThread(userID, threadID string) (bool, error) {
	var count int64
	err := r.db.Model(&Like{}).Where("user_id = ? AND thread_id = ?", userID, threadID).Count(&count).Error
	return count > 0, err
}
...
func (r *likeRepositoryImpl) HasUserLikedReply(userID, replyID string) (bool, error) {
	var count int64
	err := r.db.Model(&Like{}).Where("user_id = ? AND reply_id = ?", userID, replyID).Count(&count).Error
	return count > 0, err
}
...
func (r *likeRepositoryImpl) GetThreadLikeCount(threadID string) (int64, error) {
	var count int64
	err := r.db.Model(&Like{}).Where("thread_id = ?", threadID).Count(&count).Error
	return count, err
}
...
func (r *likeRepositoryImpl) GetReplyLikeCount(replyID string) (int64, error) {
	var count int64
	err := r.db.Model(&Like{}).Where("reply_id = ?", replyID).Count(&count).Error
	return count, err
}
...
func (r *repostRepositoryImpl) DeleteRepost(userID, threadID string) error {
	return r.db.Where("user_id = ? AND thread_id = ?", userID, threadID).Delete(&Repost{}).Error
}

// HasUserReposted implements RepostRepository
func (r *repostRepositoryImpl) HasUserReposted(userID, threadID string) (bool, error) {
	var count int64
	err := r.db.Model(&Repost{}).Where("user_id = ? AND thread_id = ?", userID, threadID).Count(&count).Error
	return count > 0, err
}
...
func (r *repostRepositoryImpl) GetRepostCount(threadID string) (int64, error) {
	var count int64
	err := r.db.Model(&Repost{}).Where("thread_id = ?", threadID).Count(&count).Error
	return count, err
}
...
func (r *bookmarkRepositoryImpl) DeleteBookmark(userID, threadID string) error {
	return r.db.Where("user_id = ? AND thread_id = ?", userID, threadID).Delete(&Bookmark{}).Error
}
...
func (r *bookmarkRepositoryImpl) HasUserBookmarked(userID, threadID string) (bool, error) {
	var count int64
	err := r.db.Model(&Bookmark{}).Where("user_id = ? AND thread_id = ?", userID, threadID).Count(&count).Error
	return count > 0, err
}

// GetUserBookmarks implements BookmarkRepository
func (r *bookmarkRepositoryImpl) GetUserBookmarks(userID string, page, limit int) ([]*Thread, int64, error) {
	var threads []*Thread
	var totalCount int64

	offset := (page - 1) * limit

	if err := r.db.Model(&Bookmark{}).Where("user_id = ?", userID).Count(&totalCount).Error; err != nil {
		return nil, 0, err
	}
...
func (r *categoryRepositoryImpl) FindOrCreateCategoryByName(name string, categoryType string) (*Category, error) {
...
func (r *categoryRepositoryImpl) FindCategoryByID(id string) (*Category, error) {
	var category Category
...
func (r *categoryRepositoryImpl) FindCategoryByName(name string, categoryType string) (*Category, error) {
	var category Category
	err := r.db.Where("name = ? AND type = ?", name, categoryType).First(&category).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
...
func (r *categoryRepositoryImpl) RemoveCategoryFromThread(threadID, categoryID string) error {
	return r.db.Where("thread_id = ? AND category_id = ?", threadID, categoryID).Delete(&ThreadCategory{}).Error
}

// FindCategoriesByThreadID implements CategoryRepository
func (r *categoryRepositoryImpl) FindCategoriesByThreadID(threadID string) ([]*Category, error) {
...
func (r *pollRepositoryImpl) CreatePoll(poll *Poll) error {
	return r.db.Create(poll).Error
}

// CreatePollOptions implements PollRepository
func (r *pollRepositoryImpl) CreatePollOptions(options []*PollOption) error {
	return r.db.Create(options).Error
}

// FindPollByID implements PollRepository
func (r *pollRepositoryImpl) FindPollByID(id string) (*Poll, error) {
	var poll Poll
...
func (r *pollRepositoryImpl) FindPollByThreadID(threadID string) (*Poll, error) {
	var poll Poll
...
func (r *pollRepositoryImpl) FindPollOptionByID(id string) (*PollOption, error) {
...
func (r *pollRepositoryImpl) FindPollOptionsByPollID(pollID string) ([]*PollOption, error) {
...
func (r *pollRepositoryImpl) CreateVote(vote *PollVote) error {
	return r.db.Create(vote).Error
}

// DeleteVote implements PollRepository
func (r *pollRepositoryImpl) DeleteVote(userID, pollID string) error {
	return r.db.Where("user_id = ? AND poll_id = ?", userID, pollID).Delete(&PollVote{}).Error
}

// FindVoteByUserAndPoll implements PollRepository
func (r *pollRepositoryImpl) FindVoteByUserAndPoll(userID, pollID string) (*PollVote, error) {
	var vote PollVote
